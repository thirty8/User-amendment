...
const SafeCustodyRegisterEnquiry = async (req, res) => {
  const {
    fetch_data,
    fetch_data2,
    account_name,
  ...
  try {
    ...
    if (acct_link) {
      determinant += `WHERE a.ACCT_LINK LIKE '%${acct_link}%'`;
    } else {
      determinant += `WHERE a.ACCT_LINK LIKE '%${acct_link}%'`;
    }

    // if (account_name) {
    //   determinant += `AND GET_ACCTDESC(a.ACCT_LINK) LIKE '%${account_name}%'`;
    // } else {
    //   determinant += "";
    // }
    if (account_name) {
      ...
      //   determinant += "";
      determinant += `AND GET_ACCTDESC(a.ACCT_LINK) LIKE '%${account_name}%'`;
      //   determinant += `WHERE GET_ACCTDESC(a.ACCT_LINK) LIKE '%${account_name}%'`;
    }
    ...
    if (fetch_data === "true") {
      const data = await con.execute(
        `SELECT a.REQUISITION_NO, a.ACCT_LINK, a.POSTED_BY, GET_ACCTDESC(a.ACCT_LINK) AS ACCOUNT_DESCRP 
         FROM TB_SAFECUSTODY_RGT a ${determinant}`
      );
...
module.exports = { SafeCustodyRegisterEnquiry };
