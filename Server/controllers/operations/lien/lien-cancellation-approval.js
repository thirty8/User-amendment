...
const LienCancellationApproval = async (req, res) => {
  const {
    fetch_data,
    req_no_v,
    prc_type_v,
  ...
  try {
    ...
    if (okay_procedure === "true") {
      ...
      const result = await con.execute(
        `BEGIN cbxdmx.prc_lien_cancapp(:prc_type_v, :req_no_v,:account_number , :amount , :global_branch_v , :expiry_date, :posted_by, :ip_address, 
    ...
    if (fetch_data === "true") {
      const data = await con.execute(
        `SELECT ACCT_LINK, ACCOUNT_NAME, LIEN_AMOUNT, LIEN_NUMBER, EXPIRY_DATE, EFFECTIVE_DATE, NEXT_REVIEW_DATE, COMMENTS FROM VW_LIENCANCEL_APP  WHERE LIEN_NUMBER = :req_no_v`,
        {
          req_no_v,
        }
      );
...
module.exports = { LienCancellationApproval };
