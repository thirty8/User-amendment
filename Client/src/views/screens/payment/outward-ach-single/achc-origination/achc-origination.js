...
function AchcOrigination() {
  ...
  axios
    .get(API_SERVER + "/api/ach/receivingbanks", {
      headers: headers,
    })
    .then(function (response) {
      for (var i = 0; i < response.data.length; i++) {
        receivingbankslovs.push({
  ...
  const [sourceAccountDefaultValue, setSourceAccountDefaultValue] =
    useState("");
  useEffect(() => {
    setAccountName(accountDetails?.account_name);
    // setACHLimit(accountDetails?.currency);
  }, [accountDetails]);
                          ...
                          >
                            <div className="flex items-center justify-between mx-2 p-2">
                              ...
                              <div
                                ...
                                onClick={() =>
                                  setShowSignatureVerfication(false)
                                }
                              >
                                x
                              </div>
