...
function ListOfAccounts({ clearModal, reportName, formatDate }) {
  const headers = {
    ...
    try {
      ...
      async function branchFunc() {
        return await axios.post(
          API_SERVER + "/api/list-of-accounts",
      ...
      async function productFunc() {
        return await axios.post(
          API_SERVER + "/api/list-of-accounts",
          { product: "true", product_code: "" },
          { headers }
        );
      }

      Promise.all([branchFunc(), productFunc()]).then((response) => {
        setBranchLOV(response[0].data);
        setProductLOV(response[1].data);
      });
    } catch (err) {
    ...
    await axios
      .post(
        API_SERVER + "/api/list-of-accounts",
        {
          ...
          product_code: myObj ? myObj?.product_code : "",
        },
        { headers }
      )
      .then((response) => {
        // console.log(response, "lexis");
        const arr = [];
        let sum = 0;
        const newArray = [];
        if (response?.data?.length > 0) {
          response?.data?.map((i) => {
            sum += i?.data?.length;
            const splitted = i?.item_descrp;
            arr.push([
              <span className="font-semibold text-left">{`${splitted}`}</span>,
              "",
              "",
              "",
              "",
            ...
            i?.data?.map((i) => {
              const { acct_link, account_descrp, balance, date_opened } = i;
              arr.push([
                "",
                acct_link,
                account_descrp,
  ...
  function generateExcel() {
    ...
    XLSX.writeFile(workbook, "All_list_of_accounts_oca.xlsb");
  }
  ...
  const handleOnChange = (name, value, name2, data) => {
    ...
    console.log(selectedOption, "bro");
    if (data?.length === 0) {
      setMyObj((prev) => ({
        ...prev,
        [name]: value,
        ...
        >
          ...
          <div className="mb-4 pt-1">
            <ListOfValue
              ...
              data={productLOV}
              onChange={(value) =>
                handleOnChange("product_code", value, null, [])
              }
              value={myObj ? myObj?.product_code : ""}
            />
          </div>
                  ...
                  <div className="mx-1" style={{ zoom: 0.9 }}>
                    <div className="sticky top-0 w-full">
                      <Header
                        ...
                        title={"List of account opened, closed & amended"}
                      />
                    </div>
