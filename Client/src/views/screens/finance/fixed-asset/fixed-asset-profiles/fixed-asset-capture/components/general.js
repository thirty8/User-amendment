...
function General({setAssetID,setGeneralDetails,setMappedAccountsArrays,clearAll}) {
    ...
    const depreciationValuesCalculator = (acqCost,sValue,usefulUnit) =>{
        ...
        if (usefulUnit === "Y")
        {
            ...
            pyd = depAmount/details.usefulLife
            pmd = pyd/12 ;
            ...
            depAmount = acqCost - sValue;
            pyd = depAmount/(details.usefulLife/12)
            pmd = pyd/12
            totalDp = pmd * details.usefulLife

            if (details.usefulLife < 12) {
                PYD = (pyd / 12) * details.usefulLife
            }
            else{
                PYD = pyd
            }
    ...
    useEffect(()=>{
        setGeneralDetails(
            ...
            // mainAssetID: assetClass,
             ...
             barCode:barCode,
             poNumber: poNumber,
             usefulLife: details?.usefulLife ,
             usefulLifeUnit: details?.usefulLifeUnit,
             rate : details?.rate ,
    ...
    },[assetCategory,assetDescription,shortDescription,mainAsset,assetLabel,assetStatus,propertyType,compMainID,acquisitionCost,depreciableAmount,salvageValue,sourceDocument,acquisitionDate,serialNumber,voucherNumber,barCode,poNumber,propertyType,details,assetClass,perYearDep,perMonthDep,totalDep])
...
console.log(endDate, "endDateendDate")
  ...
  useEffect(()=>{
    if (details.usefulLife){
    depreciationValuesCalculator(acquisitionCost,salvageValue,usefulLifeUnit);
    getEndDate(details.usefulLife,usefulLifeUnit)
    }
  },[startDate,details.usefulLife,usefulLifeUnit])
  ...
  const clearGeneralFields = () =>{
    ...
    setPerMonthDep("");
    setTotalDep("");
    document.getElementById("depStartEnd").value = "";
    document.getElementById("perYearDep").value = "";
    document.getElementById("totalDep").value = "";
    setUsefulLife("");

    // Object.keys(details)?.length < 1 ? details.currencyCode == " " && details.currency == " " : null;
  ...
  console.log(details,details.usefulLife,usefulLife,"detailsdele")
...
<div style={{display:"flex",gap:"40px",paddingTop:"3px"}}>
    ...
    <div style={{margin:"0px 0px 0px 20px"}}>
        ...
        <InputField 
                ...
                required
                id={"assetClass"}
                value={Object.values(details)?.length > 0 ? details.assetClass + "  -  " + details.assetDescription : " "}
            />
        <InputField 
                label={"Currency"}
                labelWidth={"30.8%"}
                ...
                disabled
                required
                value={Object.keys(details)?.length > 0 ? details.currencyCode + "  -  " + details.currency : " "}
            />
        </div>
        {/* <br/> */}
        {/* <hr/> */}
        ...
        <InputField 
            ...
            onChange={(e)=>{setAssetClass(e.target.value)}}
            disabled
            value={Object.keys(details)?.length > 0 ? details.mainAssetID : " "}
    /> 
        }
        <ListOfValue 
                label={"Asset Status"}
     ...
     <InputField 
                ...
                value={acquisitionDate ? acquisitionDate : details.acquisitionDate}
            />
        <InputField 
                ...
                required
                value={acquisitionCost.length > 0 ? formatNumber(depreciableAmount) : null}
                textAlign={"right"}
            />
     <InputField 
                label={"Serial Number"}
    ...
    <InputField 
                ...
                required
                value={details ? details.usefulLife : usefulLife}
                 textAlign={"center"}
            />
 <RadioButtons 
                display={true}
     ...
     <InputField 
                ...
                value={rate ? rate.toFixed(2) + " % " : null}
                 textAlign={"center"}
            />
     <InputField 
