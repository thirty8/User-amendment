...
import FixedAssetApprovalModal from './components/fixed-asset-approval-modal';
...
function FixedAssetCaptureApproval() {
...
const getFixedAssetApprovalData = () => {
          ...
          let array = [];
          axios
            .post(API_SERVER + "/api/get-asset-capture-approval-data",
            {
            ...
            .then((response) => {
              ...
              results.map((i)=>{array.push([i.asset_id,i.asset_cat,i.serial_no,<div style={{textAlign:"left"}}>{i.description}</div>,<div style={{textAlign:"right"}}>{formatNumber(i.acq_cost)}</div>,i.acquisition_date,i.p_location,i.posted_by,<div style={{display:"flex",justifyContent:"center"}}><ButtonComponent onClick={()=>{i.posted_by !== JSON.parse(localStorage.getItem("userInfo")).username ? openGLamendment() && console.log("first") : swal({icon: "error",title:"ERR - 01342 : Approval Disallowed",text: "You cannot approve your own Entry"}) ; setAssetID(i.asset_id)}} buttonIcon={<AiOutlineEye size={18}/>}/> </div>])})
...
//   [chartGroup])
  ...
  return (
    <div style={{ padding: "0px 15px" }}>
                  <ActionButtons displayNew={'none'} displayCancel={'none'} displayView={'none'} displayDelete={'none'} displayHelp={'none'} displayAuthorise={'none'} displayOk={'none'} displayReject={'none'} onRefreshClick={clearData} onFetchClick={getFixedAssetApprovalData}/>
      ...
      <div>
        ...
        <br/>
        <div>
          <Header title={"Fixed Asset Approval"} headerShade={true}/>
      ...
      <Modal size="100%" opened={GLamendment} withCloseButton={false} trapFocus={false}>
        ...
        >
          ...
          <FixedAssetApprovalModal assetID={assetID} closeGLamendment={closeGLamendment}/>
        </div>
...
export default FixedAssetCaptureApproval;
