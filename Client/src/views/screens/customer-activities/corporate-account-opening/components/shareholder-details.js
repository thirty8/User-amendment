    ...
    <div>
      ...
      >
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm text-gray-600 dark:text-gray-300">
          ...
          <tbody>
                ...
                >
                  ...
                  <td className="text-center">
                    {data.Relation_no ??
                      ...
                      data.relation_no ?? data.RELATION_NO}
                  </td>
                  <td>{data.P_fname ?? data.first_name ?? data.FIRST_NAME}</td>
                  <td>
                    {data.P_mname == "null" ? "" : data.P_mname ?? data.surname}
                  </td>
                  <td>{data.P_sname ?? data.last_name ?? data.LAST_NAME ?? data.SURNAME}</td>
                  <td className="text-center">
                  ...
                  <td>{data.P_dob ?? data.date_of_birth ?? data.DATE_OF_BIRTH}</td>
            ...
            {relationData?.length > 0 ? (
              relationData?.filter((i)=>i?.source == "Shareholderdetails").map((data, index) => (
                <tr
                  key={index}
                  className={index % 2 === 0 ? "bg-gray-100" : "bg-sky-300"}
                  // onClick={() => handleRowClick(index)}
      ...
      <div>
        ...
        {isOpenRelationDetails && (
          <div className="modal-overlay" onClick={toggleModal}>
            <h2 className="text-2xl font-semibold mb-4 text-center uppercase">This User Exist</h2>
            <div
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="p-6 border-t border-gray-200 bg-gray-50">
            <div className="flex justify-end space-x-4">
              <Button variant="outline" color="red" onClick={onCloseForRelationValidationForm}>
                Cancel
              </Button>
              <Button
