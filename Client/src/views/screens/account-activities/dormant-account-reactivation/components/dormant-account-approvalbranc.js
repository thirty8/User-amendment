...
//     //view api to populate the table
...
//                               label={"View Fees Transactionssss."}
//                               buttonHeight={"26px"}
//                               buttonColor={"white"}
...
function DormantAccountApprovalbranch({ batchNo, accountSourceBranch, setShowModal, setApproveChanged, setApproved})
 {
      ...
      try {
        axios.post(
          ...
          { headers }
                  ...
                  if(result.data.length >0){
          result.data.map((i)=>{
            arr.push([i.ACCOUNT_DESCRP,i.ACCT_LINK,i.REASON,i.DATE_ALTERED,i.POSTED_BY,
            <div><ButtonComponent buttonWidth={"40px"} buttonIcon={<FiChevronRight />}
            onClick={() => {
              ...
              axios
                .post(
                  // api view to provide account details on selected account
                  API_SERVER + "/api/get-dormant-account-approval",
                  {
                    accountNumber: i.ACCT_LINK,
                  },
                  {
  ...
  async function dmtacct() {
    console.log(
      {
        ACCT_V: getdata.acct_link,
        BATCH_V: getdata.batch_no,
        USER_V: getdata.account_descrp,
        BRA_V: getdata.branch_desc,
        HOST_V: getdata.posted_by,
        DOC_V: getdata.document_no,
      }, 'finAllyy'
   ...
   .then(function (response) {
     ...
     console.log(response.data, "waakye");
     console.log(response.data?.MSG_CODE , "wELE");
