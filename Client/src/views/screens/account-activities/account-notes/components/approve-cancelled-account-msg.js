...
async function clsapproval(value) {
  try {
    let response = await axios.post(
      API_SERVER + "/api/account-notes-acancellation-approval",{
            branchCode : value,
      },
      { headers }
    );
...
useEffect(() => {
  ...
  async function CancelApproval(){
    ...
    try{
      axios
      .post(
        API_SERVER +"/api/accn-notes-cancelapproval",
        {
       
          count: '100',
          username: 'UNSUPPORT',
          postingDate: "18-JUN-2024",
...
return (
    ...
    <div >
         <Modal opened={isChecked} onClose={() => { setIsChecked2(false); setIsChecked(false)  }} size={"50%"}>
        <div  >
          ...
          {/* {isChecked2 === true ? ( */}
            <ActionButtons
              onAuthoriseClick={()=>CancelApproval()}
              displayCancel={"none"}
              displayDelete={"none"}
              displayFetch={"none"}
              displayHelp={"none"}
