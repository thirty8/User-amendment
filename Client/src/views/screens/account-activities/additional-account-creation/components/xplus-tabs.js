...
function MyTabs({
  ...
  useEffect(() => {
    ...
    const newTableData = Array.from({ length: parseInt(selectedNumber, 10) }, () => '');
  ...
      }
   ...
   const handleOnChange = {
     ...
     customerSegment_: async function (value) {
       const data = await axios.post(API_SERVER + "/api/get-customer-sub-segment-individual", 
       {
         segmentCode: value
        },
        {headers});
     ...
     documentRequiredType: async function(value){ //introSource
  ...
  const tabsData = [
    { value: 'default', label: 'Account Details',component: 
            <div>
              <Account_Details
                sector={sector}
                subSector={subSector}
    ...
    { value: 'tab-1', label: 'Relation/Signitory Details',
      component:
      <AccordionUsable 
...
console.log('customerDataTable::::', customerDataTable)
  ...
  <div className="">
    <hr/>
    <ActionButtons handleFinalChange={handleFinalChange} />
    <hr />
