...
import BranchBatchTransaction from "./branch-batch-transactions";
...
function BranchTransactionSummary({}) {
  const [branchAndCountryCode, setBranchAndCountryCode] = useState({});
  ...
  const getBranchTransactionAnalyzer = () => {
    let array = [];
    axios.post(API_SERVER + '/api/getBranchTransactionAnalyzer',
    {
      currency:currency
    },{headers})
    .then((response) => {
      ...
      results.map((i)=>{array.push([i.branch,i.currency,i.db_cnt,i.cr_cnt,i.total_trans,<div style={{textAlign:"right"}}>{formattedNumber(i.db_amt)}</div>,<div style={{textAlign:"right"}}>{formattedNumber(i.cr_amt)}</div>,<div style={{textAlign:"right"}}>{formattedNumber(i.max_db_amt)}</div>,<div style={{textAlign:"right"}}>{formattedNumber(i.max_cr_amt)}</div>,i.last_transtime,<div style={{display:"flex",justifyContent:"center"}}><ButtonComponent buttonIcon={<AiOutlineEye/>} onClick={()=>{setBranchAndCountryCode({branch:i.branch_code,currency:i.currency_code,branchDescription:i.branch});openBatchTrans();}} buttonWidth={"27px"} buttonHeight={"27px"} size={20}></ButtonComponent></div>])})
      setData(array)
    })
    .catch((error)=>{
      console.log(error)
  ...
  <div
        ...
        >
          ...
          <div className="bg-gray-200 rounded-b ">
            ...
            >
               <BranchBatchTransaction bbg={branchAndCountryCode}/>
            </div>
          </div>
        </div>
    </Modal>
...
export default BranchTransactionSummary;
