...
function AccordionUsable({handleBlurValidation,dynamicNumber,userDataValidation, isValid,typeOfAccount,
  setDynamicNumber,
  isLoading,
  userExists,
  refinedData,
setRefinedData,
  handleSubmitValidation, setTableDataAddress,usedAddressTypes,setUsedAddressTypes,tableDataAddress, clearError, setErrorTest, displayValidData, closeModal, isModalOpen, handleCheckValueClick, message, errors, validateInput, handleNumRowsChange, effective, onclickOnrow, handleEdit,handleDelete, generateRandomNumberString, hideRadioButtons, selectedOptions, storedFormData, accountTypes, setAccountTypes, selectedOptionJoint, tableData, handleSubmit, formData, setFormData, handleChange, customerDataTable ,customerData,response, error  
}) {
...
axios.post('http://10.203.14.195:3020/api/casa/act/create/rt').then()
  ...
  return (
    <div className="App">
      ...
      { !onclickOnrow ?
      <div className={`flex justify-end mr-2 mb-2 mt-2 ${onclickOnrow ? 'hidden' : 'Save'} `}>
        <div className='border p-2 rounded-lg bg-green-700 text-white cursor-pointer' 
