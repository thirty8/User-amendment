...
function MyTabs({
  ...
  //   // Check if dataApproval and relationData exist and have length greater than 0
  //   if (dataApproval?.relationData?.length > 0) {
  //     // Initialize an array to hold all RELATION_NO values
  //     const relationNumbers = [];

  //     // Loop through each item in relationData
  //     dataApproval.relationData.forEach(item => {
  //       // Push each RELATION_NO value into the relationNumbers array
  //       relationNumbers.push(item.RELATION_NO);
  //     });

  //     // Update getRelationNumber with the array of all RELATION_NO values
  //     setgetRelationNumber(relationNumbers);
  //     console.log("responseMessage:::Data", relationNumbers);
  //   } else {
  //     // Otherwise, set getRelationNumber to null
  //     setgetRelationNumber(null);
  //   }
  ...
  // setgetRelationNumber((dataApproval?.relationData?.length > 0 ? dataApproval?.relationData[0].RELATION_NO : null));
  ...
  const handleOnChange = {
    ...
    productGroup: function (value) {
      axios
        ...
        .then(function (response) {
          console.log("Product Sub Group",response.data, value.split("-")[0], "from me");
    ...
    productSubGroup: async function (value) {
      ...
      if (data.data.length > 0) {
        ...
        response.map((i) => {
          arr.push({ value: i.actual_code, label: i.description });
           console.log("productSubGroup",arr)
        });
  ...
  <div className="flex items-center justify-end space-x-2">
    ...
    <h1 style={{ borderRadius:'10px', color: rejectedApproval === "R" ? "red" : "green", fontWeight: "bold", border: rejectedApproval === "R" ? "1px solid red" : "1px solid green", padding: "2px" }}>
      {rejectedApproval === "R" ? "Rejected" : "Approved"}
    </h1>
    <span className="font-bold">Reason for rejection:</span>
    <p className="p-4 ml-5 border border-black rounded-md">{dataApproval?.REJ_REASON}</p>
  </div>
</div>
          ...
          {/* <div className="flex items-center justify-center">
            ...
            <h1 style={{ color: rejectedApproval === "R" ? "red" : "green", fontWeight: "bold", border: rejectedApproval === "R" ? "1px solid red" : "1px solid green", padding: "2px" }}>
             {rejectedApproval === "R" ? "Rejected" : "Approved"}
            </h1>
            <span>Reason for rejection :</span><p className="p-4 ml-5 border border-black rounded-md">{dataApproval?.REJ_REASON}</p>
          </div> */}
          <hr />
