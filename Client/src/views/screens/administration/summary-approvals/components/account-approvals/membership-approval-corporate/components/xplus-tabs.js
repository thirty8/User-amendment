...
import { Stepper, Button, Group, Modal, JsonInput } from "@mantine/core";
...
import Fverification from "../../../../../../../../components/Fverification";
import TextArea from "antd/es/input/TextArea";
import CorporateBank from "./corporate-bank";
...
function MyTabs({
  ...
  const [selectedDataCorporateDetails, setSelectedDataCorporateDetails] = useState(null);

  const handleViewClick = (data) => {
    // alert(data)
    setSelectedData(dataApproval.relationData[data]);
    setSelectedDataCorporateDetails(dataApproval.corporatDetailData[0])
  };
  ...
  console.log(selectedDataCorporateDetails, "selectedDataCorporateDetails");
  ...
  const [getRelationNumber, setgetRelationNumber] = useState("");

  // Use useEffect hook to perform side effects
  useEffect(() => {
    // Check if dataApproval and relationData exist and have length greater than 0
    if (dataApproval?.relationData?.length > 0) {
      // Update getRelationNumber with the first RELATION_NO value from relationData
      setgetRelationNumber(dataApproval.relationData[0].RELATION_NO);
      console.log(
        "responseMessage:::Data",
        dataApproval.relationData[0].RELATION_NO
      );
    } else {
      // Otherwise, set getRelationNumber to null
      setgetRelationNumber(null);
    }
  }, [dataApproval]);

  console.log("responseMessage:::Data", getRelationNumber);
  console.log("responseMessage:::", {
    p_relation: dataApproval.RELATION_NO,
    p_customerno_v: dataApproval.CUSTOMER_NUMBER,
    p_branchcode: dataApproval.BRANCH_CODE,
    p_date: dataApproval.POSTING_SYS_DATE,
    p_postedby: dataApproval.POSTED_BY,
  ...
  const tabsData = [
    { 
      value: 'default', label: 'Corporate Information',
    component:
    <CorporateBank
                ...
                dataApproval.relationData?.map((data, index) => (
                  ...
                  >
                    ...
                    <td className="flex justify-center">
                      <div
                        className="text-center border p-2 shadow-md cursor-pointer bg-sky-500 rounded-md"
                        // onClick={() => handleViewClick(index)}
                      >
                        {/* <AiOutlineEye className="text-white" /> */}
                        <ImageSignatureView id={getRelationNumber} />
                      </div>
                    </td>
  ...
  //   // Handle the rejection reason submission (e.g., send it to an API or log it)
  ...
  const handleSubmitReject = async () => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        const data = {
          customerNumber: dataApproval?.CUSTOMER_NUMBER,
          approvalStatus: "R",
        ...
        const config = {
          method: "post",
          url: "http://10.203.14.195:3320/api/account_rejection",
          headers: {
        ...
        if (response.status === 200) {
          ...
          Swal.fire({
            title: 'Error',
            text: 'Failed to process the rejection.',
            icon: 'error'
          });
        }
        setOpenedReject(false);
  ...
  return (
    <div className="" style={{ zoom: "0.80" }}>
      <div>
        ...
        {/* <Button onClick={() => setOpenedReject(true)}>Open Reject Modal</Button> */}
        <Modal
          opened={openedReject}
        ...
        >
          <TextArea
            label="Your reason for Rejection"
          ...
          <div className="flex items-center justify-between">
            <button
              className="bg-red-600 text-white px-4 py-2 rounded mt-4 hover:bg-red-700 active:bg-red-800 focus:outline-none focus:ring-2 focus:ring-red-500"
              onClick={() => setOpenedReject(false)}
            >
              Cancel
            </button>

            <button
              className="bg-blue-600 text-white px-4 py-2 rounded mt-4 hover:bg-blue-700 active:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <div className="p-1">
          ...
          <ActionButtons
            ...
            onRejectClick={() => setOpenedReject(true)}
          />
          <hr />
              ...
              <div>
                ...
                <div className="flex ">
                  ...
                  >
                    ...
                    <option value="ID">PRIVATE ENTERPRISE</option>
                    <option value="PE">PUBLIC ENTERPRISE</option>
                    <option value="PA_JT">PARASTATAL</option>
                    <option value="LOCC_JT">LOCAL COUNCIL</option>
                    <option value="GOVD_JT">GOVERNMENT DEPARTMENT</option>
                    <option value="GOVA_JT">GOVERNMENT AGENCIES</option>
                  </select>
                  {/* <select
