...
const ImageSignatureCapture = ({ id }) => {
  ...
  //       img.onload = function () {
  //           cntx.drawImage(img, 0, 0);
  //           var b64String = cvs.toDataURL(); // Do not specify image type
  //           var retstring = b64String.replace(/^data:image\/jpeg;base64,/, ''); // Remove existing prefix
  //           if (callback) {
  //               callback(retstring); // Concatenate new prefix
  //           }
  //       };
  //   };
  // }
...
const handleSubmit = async () => {
  try {
    ...
    if (Imagbase64String) {
      imageBase64 = Imagbase64String;
      console.log("Imagbase64String:",imageBase64)
    } else {
      // Check if capturedImage has a value
      if (capturedImage) {
        imageBase64 = capturedImage;
        console.log("capturedImage:",imageBase64)
      } else {
        // Handle case when both Imagbase64String and capturedImage are empty
        console.error('Error: No image data available.');
        return;
    ...
    console.log("relation number", id)
    ...
    console.log("image and signature ::: =", data);
    ...
    console.log("image and signature ::: =", JSON.stringify(response.data));
    // console.log("image and signature ::: =",response.data.status);
  ...
  if(response.data.status === "success"){
    ...
    Swal.fire({
      title: "Success!",
      text: "Signature and ID Picture Saved Successfully",
      icon: "success",
      confirmButtonText: "OK",
    });
  ...
  const handleRetake = () => {
    setCapturing(true); // Set capturing to true to allow capturing image again
    ...
    <div>
      <div className="">
        <div className="w-full border border-gray-200 shadow-lg  p-4 rounded">
              ...
              {/* <span>
                ...
                <span>
                  {carouselStep === 0 ? (
                    ...
                    <IoMdArrowRoundBack size={30} /> // Assuming you have an icon for backward
                  )}
                </span>
            </div>
          </div>
          ...
          {/* {carouselStep === 0 && ( */}
          <div className={`${carouselStep === 0 ? "block" : "hidden"}`}>
            <div  >
              ...
              {isSignatureMode && !base64String ? (
                <div className="relative w-full border-4 border-blue-500 border-dashed h-64 rounded-md bg-white  hover:shadow-gray-200 transition-all duration-200">
                  {/* <span className="animate-pulse text-blue-500  absolute top-1/3 right-[25%] font-mono">Kindly sign your signature here or upload ...</span> */}
                  <canvas
                    id="cnv"
                    name="cnv"
            ...
            <div  >
              ...
              {true ? (
                <div className="flex justify-center">
                  <div 
                  className="relative border-4 border-blue-500 border-dashed h-64 rounded-md bg-white  hover:shadow-gray-200 transition-all duration-200"
                  >
                    {/* <span className="animate-pulse text-blue-500  absolute top-1/3 right-[25%] font-mono">Kindly sign your signature here or upload ...</span> */}
                    {/* <canvas
                      id="cnv"
                    ...
                    <div className="relative w-full flex justify-center">
                      {capturedImage && <img className="absolute top-0 h-60" src={capturedImage} alt="Captured" />}
                      <video ref={videoRef} autoPlay muted style={{ width: '100%', maxWidth: '500px' }} className=" h-60" />
                    </div>
                    )}
      ...
      <div className="container mx-auto p-8 flex">
        ...
        {/* <div className="w-1/2 border border-gray-500 p-4 ml-4">
          <form
            action=""
            method="post"
            name="sigForm"
...
export default ImageSignatureCapture;
